...
namespace koinos { namespace chain {
   ...
   private:
      ...
      friend struct pack::reflector<name>;
...
#define N(X) koinos::chain::string_to_name(#X)

} } // koinos::chain

namespace std {
   template<> struct hash<koinos::chain::name> : private hash<uint64_t> {
      typedef koinos::chain::name argument_type;
